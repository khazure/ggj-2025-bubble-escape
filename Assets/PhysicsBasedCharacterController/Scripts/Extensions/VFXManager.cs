...
namespace PhysicsBasedCharacterController
{
    public class VFXManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            capsuleCollider = characterManager.GetComponent<CapsuleCollider>();
        ...
        public void ParticleJump()
        {
            if (enableVFX)
            {
                GameObject tmpObj = GameObject.Instantiate(particleJump, characterManager.transform.position - new Vector3(0f, capsuleCollider.height / 2f, 0f), Quaternion.identity);
        ...
        public void ParticleLand()
        {
            if (enableVFX)
            {
                GameObject tmpObj = GameObject.Instantiate(particleLand, characterManager.transform.position - new Vector3(0f, capsuleCollider.height / 2f, 0f), Quaternion.identity);
                tmpObj.transform.parent = this.transform;
            }
        }
